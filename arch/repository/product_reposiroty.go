...
func (pr *ProductRepository) GetProducts(filters model.ProductFilters) ([]model.Product, error) {
	query := `SELECT products_id, products_name, products_price, categorias_id, categorias_name, data_cadastro FROM prod.products`
	...
	for rows.Next() {
		...
		if err := rows.Scan(&product.Id, &product.Name, &product.Price, &product.Categoria_Id, &product.Categoria_Name, &product.Data_Cadastro); err != nil {
			return nil, err
		}
...
func (pr *ProductRepository) GetProductByID(id int) (*model.Product, error) {
	...
	err = query.QueryRow(id).Scan(&product.Id, &product.Name, &product.Price, &product.Data_Cadastro, &product.Categoria_Id, &product.Categoria_Name)
	if err != nil {
...
func (pr *ProductRepository) CreateProducts(product model.Product) (int, string, error) {
	var Id int
